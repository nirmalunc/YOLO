    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            if m.__name__ == 'SimAM':
                c1 = ch[f]
                c2 = c1
                args = []  # SimAM takes no args
            else:
                print(f"[TRACE] Layer {i} module={m.__name__ if hasattr(m, '__name__') else m} from={f}, raw args={args}")
                c1, c2 = ch[f], args[0]
                if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                    c2 = make_divisible(min(c2, max_channels) * width, 8)
                    print(f"[TRACE] After scaling: c1={c1}, c2={c2}")
        ...
        print(f"Layer {i}: {m_.__class__.__name__}, in_channels = {args[0] if args else 'N/A'}, out_channels = {args[1] if len(args) > 1 else 'N/A'}")
